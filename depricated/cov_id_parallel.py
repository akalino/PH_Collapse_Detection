...
_PC_GENERATORS = {
    ...
    "paraboloid": lambda n, d, eps, seed: generate_paraboloid_graph(n, d, _eps=eps, _seed=seed),
    "contaminated_kplane": lambda n, d, eps, seed: generate_contaminated_kplane(n, d, _eps=eps, _seed=seed),
    "contaminated_sphere": lambda n, d, eps, seed: generate_noisy_sphere(n, d, _eps=eps, _seed=seed),
...
def build_cov_id_metrics_csv(
    ...
    for n, d, e, seed in product(n_list, d_list, eps_list, seeds):
        ...
        tasks.append(("base_gauss", n, d, e, seed))
        tasks.append(("base_noisy_sphere", n, d, e, seed))
        # alternatives sweep eps grid
        for pc in [
            "alt_k_plane", "contaminated_k_plane", "alt_k_cube",
            "alt_spike_gauss", "alt_parab", "alt_torus", "alt_swiss"
        ]:
